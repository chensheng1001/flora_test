...
# gpu memory log @cs
writer = SummaryWriter(log_dir="./runs/Adamw_Accelerator")
def log_mem(stage, step):
    allocated = torch.cuda.memory_allocated() / 1024 ** 2  # MB
    reserved = torch.cuda.memory_reserved() / 1024 ** 2    # MB
    max_allocated = torch.cuda.max_memory_allocated() / 1024 ** 2
    max_reserved = torch.cuda.max_memory_reserved() / 1024 ** 2
    writer.add_scalar(f"{stage}/memory_allocated_MB", allocated, step)
    writer.add_scalar(f"{stage}/memory_reserved_MB", reserved, step)
    writer.add_scalar(f"{stage}/memory_max_allocated_MB", max_allocated, step)
    writer.add_scalar(f"{stage}/memory_max_reserved_MB", max_reserved, step)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                outputs = model(**batch)
                log_mem("forward", global_step)
                loss = outputs.loss
                # We keep track of the loss at each epoch
                if args.with_tracking:
                    total_loss += loss.detach().item()
                accelerator.backward(loss)
                log_mem("backward", global_step)
                if accelerator.sync_gradients:
                    optimizer.step()
                    log_mem("optimizer_step", global_step)
                    optimizer.zero_grad(set_to_none=True)
                    log_mem("zero_grad", global_step)
                    lr_scheduler.step()
                global_step += 1
                
                # flora优化器 + flora梯度累积
                # flora优化器 + 正常梯度累积
                # 正常的梯度累积 vs. flora的梯度累积（down_pro, up）
